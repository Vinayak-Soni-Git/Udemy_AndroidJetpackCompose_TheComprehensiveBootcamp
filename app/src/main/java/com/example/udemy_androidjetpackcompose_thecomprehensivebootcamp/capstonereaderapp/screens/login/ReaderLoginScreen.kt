...
import com.example.udemy_androidjetpackcompose_thecomprehensivebootcamp.capstonereaderapp.components.EmailInput
import com.example.udemy_androidjetpackcompose_thecomprehensivebootcamp.capstonereaderapp.components.PasswordInput
...
fun ReaderLoginScreen(navController: NavController){
    ...
    Surface(modifier = Modifier.fillMaxSize()) {
        Column(horizontalAlignment = Alignment.CenterHorizontally,
            ...
            ReaderLogo()
            if (showLoginForm.value) UserForm(loading = false, isCreateAccount = false){email, pasword->
        ...
        Row(modifier = Modifier.padding(15.dp),
            ...
            }.padding(start = 5.dp), fontWeight = FontWeight.Bold, color = MaterialTheme.colorScheme.secondaryContainer)
        }
    }
}
@Preview
...
fun UserForm(loading:Boolean = false,
    ...
    val modifier = Modifier.height(250.dp).background(MaterialTheme.colorScheme.background)
...
fun SubmitButton(textId: String, loading: Boolean, validInput: Boolean, onClick:()->Unit) {
    Button(onClick = onClick,
        ...
        shape = CircleShape) {
        if(loading) CircularProgressIndicator(modifier = Modifier.size(25.dp)) else Text(text = textId)
